...
- **LSP server** (`cmd/funny-lsp`, `internal/lsp`): a from-scratch LSP 3.17 implementation (no third-party protocol dependency), replacing the v1 scaffolding that was dropped in the v2 migration. Diagnostics (parser/module/type-checker errors, precisely positioned, with error codes), hover (types, function signatures, struct layouts, builtin/keyword docs), type-aware completion (locals/functions/structs/builtins/keywords generally, struct-fields-only right after `<expr>.`), signature help, go-to-definition (including across `import`ed files), document symbols (`fn`/`struct`/`plan`+`step` outline), and formatting (delegates to the `funny fmt` formatter). See "LSP Server" in `docs/language-manual.md` for the full capability list and known gaps (rename and find-references are not yet implemented).
